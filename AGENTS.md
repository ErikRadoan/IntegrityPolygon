...
## Big picture
- `src/main/java/dev/erikradovan/integritypolygon/IntegrityPolygon.java` is the real bootstrap: it creates `modules/`, `module-data/`, config, shared services, the TCP extender socket, Javalin, metrics, then `ModuleManager`.
...
- Browser traffic goes to Javalin (`/api/*`, `/ws/live`, `/metrics`); module dashboards are extracted from each module JAR's `web/` folder and served under `/modules/{id}/` by `src/main/java/.../core/ModuleDashboardImpl.java`.
...
- Module config is expected to round-trip through `ConfigManager`; common pattern is “load config, write defaults if empty, then reload”, as seen in `AntiBotModule`, `GeoFilteringModule`, and `ServerMonitorModule`.
...
- Examples of cross-component patterns: `server-monitor` and `spike-detector` subscribe to `profiling_report`, `account-protection` sends `show_2fa_prompt`, and `profiler` deploys its bundled `profiler-extender.jar` during `onEnable()`.
...
- Prometheus exposition is built into the root web server at `/metrics`; modules typically publish through `MetricsService` instead of opening their own endpoints.

## Repository-specific gotchas
- `server-monitor` exists under `modules/server-monitor`, but `test-env/deploy.ps1`, `build-all.sh`, `repo/modules.json`, and `repo/checksums.json` currently target `anti-bot`, `account-protection`, `identity-enforcement`, `geo-filtering`, `profiler`, and `spike-detector`; do not assume every module directory is wired into local deploy/repo flows.
- The root plugin shades/relocates Javalin, Jetty, Kotlin, JWT, and Prometheus; avoid adding module code that relies on those shaded package names directly.
